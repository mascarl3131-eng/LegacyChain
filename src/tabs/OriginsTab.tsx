...
export default function OriginsTab() {
  ...
  const dnaMatchInsights = useMemo(() => originRows
    .filter(row => row.c && row.p > 0)
    .sort((a, b) => b.p - a.p)
    .slice(0, 3)
    .map(row => ({
      country: row.c,
      percent: row.p,
      strength: row.p >= 25 ? 'strong' : row.p >= 10 ? 'medium' : 'trace',
    })), [originRows]);
  ...
  return (
    <div>
      ...
      <section style={{ background: 'rgba(0,255,209,0.03)', border: '1px solid rgba(0,255,209,0.12)', borderRadius: 10, padding: '1rem', marginBottom: '1.2rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '.45rem', color: '#00FFD1', fontSize: '.66rem', letterSpacing: '.12em', marginBottom: '.35rem' }}>
          <UsersRound size={16} /> {t('dnaMatchesTitle', lang)}
        </div>
        <div style={{ color: 'rgba(239,246,255,.38)', fontSize: '.55rem', lineHeight: 1.55, marginBottom: '.75rem' }}>{t('dnaAutoMatchesDesc', lang)}</div>
        {dnaMatchInsights.length === 0 ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: '.5rem', color: 'rgba(239,246,255,.34)', fontSize: '.6rem', padding: '.45rem 0' }}>
            <Search size={14} /> {t('dnaAutoMatchesEmpty', lang)}
          </div>
        ) : (
          <div style={{ display: 'grid', gap: '.55rem' }}>
            {dnaMatchInsights.map((item, index) => (
              <div key={`${item.country}-${index}`} style={{ border: '1px solid rgba(239,246,255,.08)', borderRadius: 8, padding: '.7rem', background: 'rgba(239,246,255,.025)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '.8rem', marginBottom: '.3rem' }}>
                  <strong style={{ color: CHART_COLORS[index % CHART_COLORS.length], fontSize: '.72rem' }}>{item.country}</strong>
                  <span style={{ color: '#00FFD1', fontSize: '.68rem' }}>{item.percent}%</span>
                </div>
                <div style={{ color: 'rgba(239,246,255,.5)', fontSize: '.56rem', lineHeight: 1.55 }}>{t(`dnaAuto_${item.strength}`, lang)}</div>
              </div>
            ))}
          </div>
        )}
      ...
      {originRows.map((row, i) => (
        <div key={i} style={{ display: 'flex', gap: '0.35rem', alignItems: 'center', marginBottom: '0.4rem' }}>
          ...
          <button type="button" aria-label={t('delete', lang)} onClick={() => removeRow(i)} style={{ background: 'transparent', border: 'none', color: 'rgba(255,45,85,0.45)', cursor: 'pointer', fontSize: '1rem', flexShrink: 0 }}>×</button>
        </div>
      ))}
